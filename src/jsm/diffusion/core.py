...
class Diffusion(L.LightningModule):
    ...
    def _compute_loss(self, batch, prefix):
        ...
        try:
            ...
            logger.error(
                'Loss compute failed; input_ids.shape=%s, dumping batch to '
                'error_batch.pkl in cwd', tuple(batch['input_ids'].shape))
    ...
    def on_train_epoch_start(self):
        ...
        logger.info('Epoch %d start', self.current_epoch)
        # number of epoch
        if self.config.enable_rna_type_prediction or self.config.enable_go_prediction:
            if self.current_epoch < self.config.enable_function_training_on_epoch:
                ...
                self.rna_type_loss_weight = 0.
                self.lm_loss_weight = 1.
                logger.info('backbone not frozen (epoch < enable_function_training_on_epoch)')
                ...
                logger.info('backbone frozen (entered function-training epoch)')
            else:
                self.go_loss_weight = self.config.go_loss_weight
                self.rna_type_loss_weight = self.config.rna_type_loss_weight
                self.lm_loss_weight = 1.
                self.backbone.unfreeze_encoder()
                logger.info('backbone unfrozen (past function-training epoch)')
            logger.info(
                'loss weights: go=%.4g rna_type=%.4g lm=%.4g',
                self.go_loss_weight, self.rna_type_loss_weight,
                self.lm_loss_weight)
