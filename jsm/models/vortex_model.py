...
class AttentionBlock(nn.Module):
    ...
    def init_weights(self):
        ...
        Residual-branch outputs (attention out_proj, MLP down_proj) use a smaller gain ~ 1/sqrt(2L).
        RMSNorm scales are set to 1. Biases (if present) are zeroed.

        Works with:
          - fused qkv (e.g., .qkv or .in_proj) or separate q_proj/k_proj/v_proj
          - MLPs with gate/up/down naming
        """
        # total number of layers for residual scaling (best effort guess)
        ...
        for name, module in self.named_modules():
            ...
            # RMSNorm (scale-only layer norm)
            elif isinstance(module, RMSNorm):
                # Many RMSNorm implementations expose .weight as the scale parameter
                if hasattr(module, "weight") and module.weight is not None:
                    nn.init.ones_(module.weight)
        
        mha = self.inner_mha_cls
        for attr in ["dwconv_qkv", "dwconv_q", "dwconv_kv"]:
            if hasattr(mha, attr) and isinstance(getattr(mha, attr), nn.Conv1d):
                conv1d_layer = getattr(mha, attr)
                nn.init.kaiming_normal_(conv1d_layer.weight, mode='fan_in', nonlinearity='relu')
                # Scale down slightly for GELU's smoother activation
                with torch.no_grad():
                    conv1d_layer.weight *= 0.87  # Empirical scaling for GELU
        
        # Handle common fused-QKV weights explicitly if present
        # (some MHA impls store qkv in a single Linear named 'qkv' or 'in_proj')
        for attr in ["qkv", "in_proj", "Wqkv", "wqkv", "Wq", "Wkv"]:
            if hasattr(mha, attr) and isinstance(getattr(mha, attr), nn.Linear):
                lin = getattr(mha, attr)
                nn.init.normal_(lin.weight, 0.0, std)
                if lin.bias is not None:
                    nn.init.zeros_(lin.bias)
        
        # And common separate projections if they exist
        ...
        # Attention out projection (scaled)
        ...
        # MLP specifics: scale the final "down" projection
        nn.init.zeros_(self.mlp.l1.weight)
        nn.init.zeros_(self.mlp.l2.weight)
        nn.init.zeros_(self.mlp.l3.weight)
    ...
    def forward(self, u, inference_params=None, padding_mask=None, *args, **kwargs):
        ...
        # B, L, D = u.shape
        # indices, cu_seqlens, max_seqlen_in_batch = get_unpad_data(padding_mask)
        # u = index_first_axis(rearrange(u, "b s ... -> (b s) ..."), indices).unsqueeze(0)
            # u = (
            #     self.inner_mha_cls(
            #         self.pre_norm(u),
            ...
            #     + u
            # )
            # u = pad_input(u.squeeze(0), indices, B, L)
...
class HyenaCascade(nn.Module):
    ...
    def _init_poles(self, method="stable", scale=1e-2):
        """Initialize poles for IIR filters."""
        
        if method == "stable":
            # Initialize poles to be stable (negative real parts)
            # Real part: negative, Imaginary part: small
            real_part = -torch.abs(torch.randn_like(self.log_poles[..., 0])) * scale
            imag_part = torch.randn_like(self.log_poles[..., 0]) * scale * 0.1
            
            # For complex poles, we need to ensure conjugate pairs for real-valued signals
            # Here we simplify by making them real
            self.log_poles.data = real_part[..., None]
            
        elif method == "critical":
            # Initialize near the edge of stability
            self.log_poles.data = torch.full_like(self.log_poles, -1e-3)
            
        elif method == "random":
            # Random initialization with given scale
            nn.init.normal_(self.log_poles, 0, scale)
            
        else:
            raise ValueError(f"Unknown pole initialization method: {method}")

    def _init_default(self, 
                    short_filter_scale=1.0,
                    log_poles_scale=1e-2,
                    residues_scale=1.0,
                    fir_inner_scale=1.0,
                    pole_init_method="stable"):
        """Default initialization with stable poles and reasonable scaling."""
        
        # Initialize short filter weights
        fan_in = self.short_filter_length
        bound = math.sqrt(3.0 / fan_in) * short_filter_scale
        nn.init.uniform_(self.short_filter_weight, -bound, bound)
        
        if self.short_filter_bias is not None:
            nn.init.zeros_(self.short_filter_bias)
        
        # Initialize FIR inner filter if present
        if self.h is not None:
            if self.fir_inner_filter_length >= 128:
                # For longer filters, use smaller initial values
                std = math.sqrt(2.0 / self.fir_inner_filter_length) * fir_inner_scale
                nn.init.normal_(self.h, 0, std)
            else:
                # For shorter filters, use uniform initialization
                bound = math.sqrt(3.0 / self.fir_inner_filter_length) * fir_inner_scale
                nn.init.uniform_(self.h, -bound, bound)
        
        # Initialize IIR parameters if present
        if hasattr(self, 'log_poles') and self.log_poles is not None:
            self._init_poles(pole_init_method, log_poles_scale)
            
            # Initialize residues
            std = math.sqrt(2.0 / self.state_size) * residues_scale
            nn.init.normal_(self.residues, 0, std)
...
class ParallelGatedConvBlock(nn.Module):
    def __init__(self, config, layer_idx, hyena_filter_groups=None, fir_inner_filter_length=None) -> None:
        ...
        self.use_fp8_input_projections = config.get("use_fp8_input_projections", False)
    ...
    def proj_norm(self, x):
        ...
        with te.fp8_autocast(enabled=self.use_fp8_input_projections):
    ...
    def init_weights(self):
        """
        Initialize weights for ParallelGatedConvBlock components.
        Uses different initialization strategies optimized for each component type.
        """
        # Initialize normalization layers (RMSNorm)
        # RMSNorm typically uses ones for scale parameter
        if hasattr(self.pre_norm, 'scale') and self.pre_norm.scale is not None:
            nn.init.ones_(self.pre_norm.scale)
        if hasattr(self.post_norm, 'scale') and self.post_norm.scale is not None:
            nn.init.ones_(self.post_norm.scale)
        
        # std = 2 / (self.layer_idx+1) / math.sqrt(self.config.hidden_size)
        std = 0.02
        # Initialize projection layer (TELinear/Linear)
        # TELinear already uses xavier_uniform_ in __init__, but we can ensure consistency
        if hasattr(self.projections, 'weight'):
            nn.init.normal_(self.projections.weight, mean=0.0, std=std)
            if hasattr(self.projections, 'bias') and self.projections.bias is not None:
                nn.init.zeros_(self.projections.bias)
        
        # Initialize output filter dense layer
        # Use scaled initialization for better gradient flow
        nn.init.normal_(self.out_filter_dense.weight, mean=0.0, std=std)
        if self.out_filter_dense.bias is not None:
            nn.init.zeros_(self.out_filter_dense.bias)
        
        # Initialize the Hyena filter cascade
        if hasattr(self.filter, 'init_weights'):
            self.filter.init_weights()
        else:
            # Fallback initialization for HyenaCascade components
            for name, module in self.filter.named_modules():
                if isinstance(module, nn.Linear):
                    # Use smaller std for filter weights to maintain stability
                    nn.init.normal_(module.weight, mean=0.0, std=std)
                    if module.bias is not None:
                        nn.init.zeros_(module.bias)
                elif isinstance(module, nn.Conv1d):
                    # For convolution layers in the filter
        ...
        if hasattr(self.mlp, 'init_weights'):
            ...
            # Fallback initialization for MLP components
            for name, module in self.mlp.named_modules():
                if isinstance(module, nn.Linear):
                    # Use Xavier/Glorot initialization for MLP layers
                    nn.init.normal_(module.weight, mean=0.0, std=std)
                    if module.bias is not None:
                        nn.init.zeros_(module.bias)
        nn.init.zeros_(self.mlp.l1.weight)
    ...
    def apply_weight_init(self, init_fn=None):
        """
        Apply custom weight initialization function to the module.
        
        Args:
            init_fn: Optional custom initialization function. If None, uses default init_weights.
        """
        if init_fn is None:
            self.init_weights()
        else:
    ...
    # Alternative initialization methods for different training scenarios
    def init_weights_small_model(self):
        """
        Weight initialization optimized for smaller models.
        Uses slightly larger initialization values.
        """
        ...
        # Projections with larger std
        if hasattr(self.projections, 'weight'):
            nn.init.normal_(self.projections.weight, mean=0.0, std=0.05)
            if hasattr(self.projections, 'bias') and self.projections.bias is not None:
                nn.init.zeros_(self.projections.bias)
        
        # Output projection
        nn.init.normal_(self.out_filter_dense.weight, mean=0.0, std=0.05)
        if self.out_filter_dense.bias is not None:
            nn.init.zeros_(self.out_filter_dense.bias)

    def init_weights_large_model(self):
        """
        Weight initialization optimized for larger models.
        Uses smaller initialization values for better stability.
        """
        # Normalization layers
        if hasattr(self.pre_norm, 'scale') and self.pre_norm.scale is not None:
            nn.init.ones_(self.pre_norm.scale)
        ...
        # Projections with smaller std
        if hasattr(self.projections, 'weight'):
            fan_in = self.projections.weight.size(1)
            std = (1.0 / fan_in) ** 0.5
            nn.init.normal_(self.projections.weight, mean=0.0, std=std)
            if hasattr(self.projections, 'bias') and self.projections.bias is not None:
                nn.init.zeros_(self.projections.bias)
        ...
        fan_in = self.out_filter_dense.weight.size(1)
        std = (1.0 / fan_in) ** 0.5 * 0.5  # Scale down for residual connection
        nn.init.normal_(self.out_filter_dense.weight, mean=0.0, std=std)
        if self.out_filter_dense.bias is not None:
            nn.init.zeros_(self.out_filter_dense.bias)

    def init_weights_zero_out(self):
        """
        Zero initialization for the output projection to start with identity mapping.
        Useful for very deep networks or when using strong residual connections.
        """
        # Normal initialization for most components
        self.init_weights()
        
        # Zero out the final output projection
        nn.init.zeros_(self.out_filter_dense.weight)
        if self.out_filter_dense.bias is not None:
            nn.init.zeros_(self.out_filter_dense.bias)
