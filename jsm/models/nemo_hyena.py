...
from jsm.models.nemo_model import BaseHyenaModelConfig, safe_init_megatron
...
class JointSequenceStripedHyenaModel(nn.Module):
    ...
    def __init__(
        ...
        hyena_config = BaseHyenaModelConfig(**config.hyena_config)
        hyena_config.params_dtype = eval(hyena_config.params_dtype)
        self.backbone = hyena_config.configure_model()
    ...
    def forward(self, 
        ...
        remainder = max_seqlen_in_batch % 8
        if remainder == 0:
            pass
        else:
            padding_len = 8 - remainder
            inputs_embeds = F.pad(inputs_embeds, (0, 0, 0, padding_len))
        hidden_states = self.backbone(
            input_ids=None,
            position_ids=None,
            decoder_input=inputs_embeds,
            # inference_params=inference_params,
            attention_mask=attention_mask[:, :max_seqlen_in_batch] if attention_mask is not None else None,
        )
        hidden_states = hidden_states[:, :max_seqlen_in_batch, :]
