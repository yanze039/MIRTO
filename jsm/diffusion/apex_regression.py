...
class APEXDiffusionRegressor(Diffusion):
    def __init__(
        self,
        config,
        global_tokenizer,
        ...
        self.ensemble_predictor = ShallowEnsembleHead(
            input_dim=self.config.model.hidden_size * 2,
    ...
    def forward(self, 
        ...
        pooled_embeddings = torch.cat(
            [pooled_utr5_embedding, pooled_utr3_embedding],
            dim=-1
        )
    ...
    def validation_step(self, batch, batch_idx):
        output = self.forward(batch)
        targets = batch['apex_label'].float()
