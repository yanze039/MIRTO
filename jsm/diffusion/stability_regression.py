...
class ShallowEnsembleHead(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_heads=5, dropout=0.0):
        super().__init__()
    ...
    def forward(self, x):
        # x: [batch, hidden] or [batch, seq_len, hidden]
        preds = torch.stack([head(x) for head in self.heads], dim=1)
        # shape:
...
class StabilityDiffusionRegressor(Diffusion):
    def __init__(
        self,
        config,
        global_tokenizer,
        ...
        self.ensemble_predictor = ShallowEnsembleHead(
            input_dim=self.config.model.hidden_size,
        ...
        self.label_names = self.config.prediction.label_names
        # Global averaged metrics across labels
        self.val_metrics = MetricCollection({
            "mae_macro": MeanAbsoluteError(num_outputs=num_labels),
            "rmse_macro": MeanSquaredError(num_outputs=num_labels, squared=False),
            "pearson_macro": PearsonCorrCoef(num_outputs=num_labels),
        })
        # Per-label metrics
        self.val_mae_per_label = MeanAbsoluteError(num_outputs=num_labels)
        self.val_rmse_per_label = MeanSquaredError(num_outputs=num_labels, squared=False)
    ...
    def forward(self, batch):
        ...
        #     [pooled_utr5_embedding, pooled_cds_embedding, pooled_utr3_embedding],
        #     dim=-1
        # )
        pooled_embeddings = pooled_utr3_embedding
    ...
    def _ensemble_prediction(self, pred):
        return pred.mean(dim=1)
    
    def validation_step(self, batch, batch_idx):
        output = self.forward(batch['rna_input_ids'], batch)
        targets = batch['stability_label'].float()
        
        losses = self._loss(output, targets)
        ensemble_preds = self._ensemble_prediction(output)
        # update metrics over full validation set
        self.val_metrics.update(ensemble_preds.float(), targets.float())
        self.val_mae_per_label.update(ensemble_preds.float(), targets.float())
        self.val_rmse_per_label.update(ensemble_preds.float(), targets.float())
        self.val_pearson_per_label.update(ensemble_preds.float(), targets.float())
        self.val_r2_per_label.update(ensemble_preds.float(), targets.float())
    ...
    def on_validation_epoch_end(self):
        # global metrics
        mae_vec = self.val_mae_per_label.compute()        # [C]
        rmse_vec = self.val_rmse_per_label.compute()      # [C]
        pearson_vec = self.val_pearson_per_label.compute()# [C]
        r2_vec = self.val_r2_per_label.compute()          # [C]
        ...
        if len(self.label_names) == 1:
            ...
            per_label_logs[f"val_per_label/mae_{name}"] = mae_vec.item()
            per_label_logs[f"val_per_label/rmse_{name}"] = rmse_vec.item()
            per_label_logs[f"val_per_label/pearson_{name}"] = pearson_vec.item()
            per_label_logs[f"val_per_label/r2_{name}"] = r2_vec.item()
        else:
        ...
        print("\nPer-label validation regression metrics:")
        if len(self.label_names) == 1:
            name = self.label_names[0]
            print(
                    f"{name:>20s} | "
                    f"MAE={float(mae_vec.item()):.4f} | "
                    f"RMSE={float(rmse_vec.item()):.4f} | "
                    f"R2={float(r2_vec.item()):.4f} | "
                    f"Pearson={float(pearson_vec.item()):.4f}"
                )
        else:
            for i, name in enumerate(self.label_names):
                print(
                    f"{name:>20s} | "
                    f"MAE={float(mae_vec[i]):.4f} | "
                    f"RMSE={float(rmse_vec[i]):.4f} | "
                    f"R2={float(r2_vec[i]):.4f} | "
                    f"Pearson={float(pearson_vec[i]):.4f}"
                )
