...
class CellTypeSpecificJointSequenceLMDBSequenceDataset(Dataset):
    def __init__(
            self, 
            lmdb_path, 
            keys=None,
        ...
        if keys is None:
            raise ValueError("keys must be provided for CellTypeSpecificJointSequenceLMDBSequenceDataset")
        else:
    ...
    def __getitem__(self, idx):
        ...
        cell_type, seq_id = self.keys[idx]  # Get the sequence ID
        ...
        return seq_id, protein_sequence, utr5_sequence, cds_sequence, utr3_sequence, cell_type
    ...
    def get_batch_indices(self, 
        ...
        for i, (ct, kk) in enumerate(self.keys):
            sizes.append(
                (
...
class CellTypeSpecificJointSequenceBatchConverter(object):
    ...
    def __init__(
            ...
            cell_type_list=["ebv_lymphocytes", "liver_hepatocyte"]
        ):
        ...
        self.cell_type_to_idx = {ct: i for i, ct in enumerate(self.cell_type_list)}
    ...
    def __call__(self, 
        ...
        for batch_idx, (label, protein_sequence, utr5_sequence, cds_sequence, utr3_sequence, cell_type ) in enumerate(raw_batch):
            ...
            modality_type_tensors[
                batch_idx,
                3 + len(utr_5_tensor) : 4 + len(utr_5_tensor) + len(codon_tensor) + 1
            ...
            modality_type_tensors[
                batch_idx,
                4 + len(utr_5_tensor) + len(codon_tensor) + 1 : 6 + len(utr_5_tensor) + len(codon_tensor) + len(utr_3_tensor) + 1
            ...
            modality_type_tensors[batch_idx, 7 + len(utr_5_tensor) + len(codon_tensor) + len(utr_3_tensor)] = self.modality_map["others"]
        ...
        cell_type_ids = torch.tensor([self.cell_type_to_idx[ct] for ct in all_cell_types], dtype=torch.int32)
        cell_type_ids = cell_type_ids.reshape(-1, 1)
        ...
        # no cell type padding mask needed since all are valid
        cell_type_padding_mask = torch.ones((batch_size, 1), dtype=torch.long)
        L = rna_padding_mask.shape[1] + protein_padding_mask.shape[1] + cell_type_padding_mask.shape[1]
        joint_masking = torch.cat([cell_type_padding_mask, protein_padding_mask, rna_padding_mask], dim=1)        
...
class CellTypeSpecificJointSequenceDataModule(pl.LightningDataModule):
    def __init__(
            self, 
            lmdb_path: str, 
        ...
        self.protein_tokenizer = protein_tokenizer
        # get offsets
        self.batch_converter = CellTypeSpecificJointSequenceBatchConverter(
    ...
    def setup(self, stage: Optional[str] = None):      
        ...
        if stage == "fit" or stage == "validate" or stage is None:
            self.training_dataset = CellTypeSpecificJointSequenceLMDBSequenceDataset(self.lmdb_path, keys=training_key_list)
            self.validation_dataset = CellTypeSpecificJointSequenceLMDBSequenceDataset(self.lmdb_path, keys=validation_key_list)
            if self.sequence_length_file is not None:
        ...
        if stage == "test":
            ...
            self.test_dataset = CellTypeSpecificJointSequenceLMDBSequenceDataset(self.lmdb_path, keys=test_key_list)
            if self.sequence_length_file is not None:
