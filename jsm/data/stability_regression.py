...
class StabilityJointSequenceDataset(Dataset):
    def __init__(
            self, 
            data_file_path,
            metadata_path,
        ...
        loc_columns = ['normalized_half_life']
        self.loc_columns = loc_columns
        self.data = self.data[["utr5_sequence", "cds_sequence", 'best_refseq_mrna_id',
                               "utr3_sequence", "protein_sequence"] + self.loc_columns]
        self.data.set_index("best_refseq_mrna_id", inplace=True)
    ...
    def __getitem__(self, idx):
        ...
        refseq_id = self.keys[idx]  # Get the sequence ID
        row = self.data.loc[refseq_id]
...
class StabilitySpeciesSpecificJointSequenceBatchConverter(object):
    ...
    def __call__(self, 
        ...
        loc_tensors = torch.empty((batch_size, 1), dtype=torch.int64)
...
class StabilitySpeciesSpecificJointSequenceDataModule(pl.LightningDataModule):
    def __init__(
            self, 
            stability_data_path: str,
            metadata_path: str,
        ...
        self.batch_converter = StabilitySpeciesSpecificJointSequenceBatchConverter(
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == "fit" or stage == "validate" or stage is None:
            # rnaloc dataset
            self.training_dataset = StabilityJointSequenceDataset(self.stability_data_path, self.metadata_path, mode='train')
            self.validation_dataset = StabilityJointSequenceDataset(self.stability_data_path, self.metadata_path, mode='val')
        
        if stage == "test":
            self.test_dataset = StabilityJointSequenceDataset(self.stability_data_path, self.metadata_path, mode='test')
