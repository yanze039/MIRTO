...
class DPOUTR5SpeciesSpecificJointSequenceSequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = self.data.loc[seq_id, :]
        utr5_sequence_chosen = sample['utr5_sequence']
        
        # 70% chance to introduce repeating sequence
        if random.random() < 0.7:
            utr5_sequence_rejected = self.mock_repeating_sequence(utr5_sequence_chosen)
        else:
            utr5_sequence_rejected = self.mock_empty_or_short_sequence(max_length=2)
    ...
    def mock_repeating_sequence(self, sequence):
        ...
        repeating_length = random.randint(5, 12)
        # randomly mask a segment in the sequence with the repeating nt
        # 20% chance as the starting position
        if random.random() < 0.2 or len(sequence) <= repeating_length:
            start_idx = 0
        else:
