...
class TEJointSequenceDataset(Dataset):
    def __init__(
            self, 
            data_file_path,
            metadata_path,
        ...
        loc_columns = ['108T', '12T', 'A2780', 'A549', 'BJ', 'BRx-142', 'C643', 'CRL-1634',
...
class TESpeciesSpecificJointSequenceBatchConverter(object):
    ...
    def __call__(self, 
        ...
        loc_tensors = torch.empty((batch_size, 78), dtype=torch.int64)
        ...
        return {
            ...
            "te_label": loc_tensors
        }


class TESpeciesSpecificJointSequenceDataModule(pl.LightningDataModule):
    def __init__(
            self, 
            te_data_path: str,
            metadata_path: str,
        ...
        self.batch_converter = TESpeciesSpecificJointSequenceBatchConverter(
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == "fit" or stage == "validate" or stage is None:
            # rnaloc dataset
            self.training_dataset = TEJointSequenceDataset(self.te_data_path, self.metadata_path, mode='train')
            self.validation_dataset = TEJointSequenceDataset(self.te_data_path, self.metadata_path, mode='val')
        
        if stage == "test":
            self.test_dataset = TEJointSequenceDataset(self.te_data_path, self.metadata_path, mode='test')
