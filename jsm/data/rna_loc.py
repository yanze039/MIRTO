...
class RNALocJointSequenceDataset(Dataset):
    def __init__(
            self, 
            data_file_path,
            metadata_path,
        ...
        self.data['transcript_length'] = self.data['protein_sequence'].str.len() + \
            self.data['utr5_sequence'].str.len() + self.data['cds_sequence'].str.len() + \
        ...
        self.loc_columns = sorted(loc_columns)
...
class RNALocSpeciesSpecificJointSequenceBatchConverter(object):
    ...
    def __call__(self, 
        ...
        loc_tensors = torch.empty((batch_size, 7), dtype=torch.int64)
...
class RNALocSpeciesSpecificJointSequenceDataModule(pl.LightningDataModule):
    def __init__(
            self, 
            rnaloc_data_path: str,
            metadata_path: str,
        ...
        super().__init__()
        self.rnaloc_data_path = rnaloc_data_path
        ...
        self.batch_converter = RNALocSpeciesSpecificJointSequenceBatchConverter(
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == "fit" or stage == "validate" or stage is None:
            # rnaloc dataset
            self.training_dataset = RNALocJointSequenceDataset(self.rnaloc_data_path, self.metadata_path, mode='train')
            self.validation_dataset = RNALocJointSequenceDataset(self.rnaloc_data_path, self.metadata_path, mode='val')
        
        if stage == "test":
            self.test_dataset = RNALocJointSequenceDataset(self.rnaloc_data_path, self.metadata_path, mode='test')
