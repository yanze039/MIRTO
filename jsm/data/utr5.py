...
def broadcast_error_flag(error_flag):
    # Broadcast 0 or 1
    error_tensor = torch.tensor([error_flag], dtype=torch.int32)
...
class JointSequenceLMDBSequenceDataset(Dataset):
    def __init__(
            self, 
            lmdb_path, 
            keys=None,
        ...
        self.env = lmdb.open(self.lmdb_path, readonly=True, lock=False, readahead=True, meminit=False, subdir=False)
    ...
    def __getitem__(self, idx):
        ...
        return seq_id, protein_sequence, utr5_sequence, cds_sequence, utr3_sequence
        ...
        for i, kk in enumerate(self.keys):
            sizes.append(
                (
                    ...
                    min(self.sequences_length[kk][1], self.rna_max_length),
                    i
...
class JointSequenceBatchConverter(object):
    ...
    def __call__(self, 
        ...
        max_len = max(len(utr5_sequence)  \
                    for _, _, utr5_sequence, cds_sequence, utr3_sequence in raw_batch)
        ...
        tokens = torch.empty(
            (
                ...
                + 2 * 1  # <cls> and <eos> for each modality, serving as boundary
            ),
            dtype=torch.int64,
        )
        ...
        for batch_idx, (label, protein_sequence, utr5_sequence, cds_sequence, utr3_sequence ) in enumerate(raw_batch):
            ...
            modality_type_tensors[batch_idx, 0] = self.modality_map["others"]
            ...
            modality_type_tensors[
                batch_idx,
                1:2 + len(utr_5_tensor) + 1
            ] = self.modality_map["utr_5"]
            # global eos
            
            tokens[batch_idx, 3 + len(utr_5_tensor)] = self.global_tokenizer.eos_idx
            modality_type_tensors[batch_idx, 3 + len(utr_5_tensor)] = self.modality_map["others"]
        ...
        joint_masking = torch.cat([protein_padding_mask, rna_padding_mask], dim=1)        
...
class JointSequenceDataModule(pl.LightningDataModule):
    def __init__(
            self, 
            lmdb_path: str, 
        ...
        self.protein_tokenizer = protein_tokenizer
        # get offsets
        self.batch_converter = JointSequenceBatchConverter(
    ...
    def setup(self, stage: Optional[str] = None):      
        if os.path.exists(self.training_key_list_file) and \
            ...
            # self.training_key_list_file = self.work_dir / "training_key_list.msgpack.zst"
            # self.validation_key_list_file = self.work_dir / "validation_key_list.msgpack.zst"  
            # if Path(self.key_list_file).suffix == ".pkl":
            #     with open(self.key_list_file, "rb") as fp:
            #         key_list = pickle.load(fp)
            # elif Path(self.key_list_file).suffix == ".zst":
            #     key_list = read_compressed_msgpack(self.key_list_file)
            # else:
            #     key_list = read_file_to_list(self.key_list_file)
            # random.shuffle(key_list)
            # split_boundary = int(len(key_list) * (1-self.validation_split))
            # training_key_list = key_list[:split_boundary]
            # validation_key_list = key_list[split_boundary:]
            # if (not dist.is_initialized()) or is_main_process():
            #     if not os.path.exists(self.work_dir):
            #         os.makedirs(self.work_dir, exist_ok=True)
            #     save_compressed_msgpack(self.training_key_list_file, training_key_list)
            #     save_compressed_msgpack(self.validation_key_list_file, validation_key_list)
            #     print(f"training_key_list_file saved to {self.training_key_list_file}")
            #     print(f"validation_key_list_file saved to {self.validation_key_list_file}")
        ...
        if stage == "fit" or stage == "validate" or stage is None:
            self.training_dataset = JointSequenceLMDBSequenceDataset(self.lmdb_path, keys=training_key_list)
            self.validation_dataset = JointSequenceLMDBSequenceDataset(self.lmdb_path, keys=validation_key_list)
            if self.sequence_length_file is not None:
        ...
        if stage == "test":
            ...
            self.test_dataset = JointSequenceLMDBSequenceDataset(self.lmdb_path, keys=test_key_list)
            if self.sequence_length_file is not None:
    ...
    def get_dataloader(self, dataset):
        if self.toks_per_batch is None:
            print(">>>>>> Using DistributedBucketBatchSampler for distributed training")
            # distributed_bucket_sampler = DistributedBucketBatchSampler(batch_indices)
            ...
            # print("NOTE: toks_per_batch is None, using batch_size instead")
            # return DataLoader(
            #         dataset, 
            ...
            if dist.is_initialized():
                ...
                # distributed_bucket_sampler = DistributedBucketBatchSampler(batch_indices)
                distributed_bucket_sampler = DistributedSequenceBucketBatchSampler(
                    dataset, toks_per_batch=self.toks_per_batch,
                )
                ...
                print("Using regular DataLoader for non-distributed training")
                batch_indices = dataset.get_batch_indices(self.toks_per_batch, 
                                                    extra_toks_per_seq=8,
                                                    sort=True,
                                                    shuffle_batch=True)
                return DataLoaderWrapper(dataset, 
...
def prepare_inputs_for_model(
    ...
    with torch.no_grad():
        ...
        prompt_input_ids = torch.tensor([prompt_token], device=device, dtype=torch.int64).view(1, 1)
...
def tokenize_inputs(
    ...
    protein_embeddings, _ = prepare_inputs_for_model(
        protein_sequence,
        protein_tokenizer,
        protein_encoder,
        global_tokenizer
    )
    tokens = torch.zeros((1, 8 + len(utr5_sequence) + len(cds_sequence) + len(utr3_sequence)))
    ...
    # global eos
    tokens[0, 7 + len(utr_5_tensor) + len(codon_tensor) + len(utr_3_tensor)] = global_tokenizer.eos_idx
