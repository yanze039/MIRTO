...
class APEXJointSequenceDataset(Dataset):
    def __init__(
            self, 
            data_file_path,
            metadata_path,
        ...
        loc_columns = ['Nucleus', 'Nucleolus', 'Lamina', 'Nuclear_Pore', 'Cytosol', 'ERM', 'OMM', 'ER_Lumen']
...
class APEXSpeciesSpecificJointSequenceBatchConverter(object):
    ...
    def __call__(self, 
        ...
        loc_tensors = torch.empty((batch_size, 8), dtype=torch.int64)
...
class APEXSpeciesSpecificJointSequenceDataModule(pl.LightningDataModule):
    def __init__(
            self, 
            apex_data_path: str,
            metadata_path: str,
        ...
        super().__init__()
        self.apex_data_path = apex_data_path
        ...
        self.batch_converter = APEXSpeciesSpecificJointSequenceBatchConverter(
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == "fit" or stage == "validate" or stage is None:
            # rnaloc dataset
            self.training_dataset = APEXJointSequenceDataset(self.apex_data_path, self.metadata_path, mode='train')
            self.validation_dataset = APEXJointSequenceDataset(self.apex_data_path, self.metadata_path, mode='val')
        
        if stage == "test":
            self.test_dataset = APEXJointSequenceDataset(self.apex_data_path, self.metadata_path, mode='test')
